...
class ModalHeader extends Component{
	...
	render() {
		return(
			<div className="spl-modal-header-navigation" style={{ 'background-color': `${(this.props.primary_color)}` }}>
				<a className={ "spl-navigation " + ( this.props.isAnswer? 'spl-active' : 'spl-inactive' ) } onClick={ () => { this.handleAnswer( true ) } } style={{ 'color': this.props.isAnswer? `${(this.props.primary_color)}` : 'white' }}>
					<div className="spl-navigation-icon">
						<svg height="24px" width="24px" style={{ 'fill': this.props.isAnswer? `${(this.props.primary_color)}` : 'white' }}>
							<path fill-rule="evenodd" d="M6 11.5C6 9.019 8.019 7 10.5 7S15 9.019 15 11.5 12.981 16 10.5 16A4.505 4.505 0 016 11.5m13.707 7.793l-3.965-3.966A6.456 6.456 0 0017 11.5C17 7.916 14.084 5 10.5 5A6.508 6.508 0 004 11.5c0 3.584 2.916 6.5 6.5 6.5a6.455 6.455 0 003.828-1.259l3.965 3.966a.997.997 0 001.414 0 .999.999 0 000-1.414"></path>
						</svg>
					</div>
				...
				</a>
				{
					...
					<a className={ "spl-navigation " + ( this.props.isAnswer? 'spl-inactive' : 'spl-active' ) } onClick={ () => { this.handleAsk( false ) } } style={{ 'color': this.props.isAnswer? 'white' : `${(this.props.primary_color)}` }}>
						<div className="spl-navigation-icon">
							<svg height="24px" width="24px" style={{ 'fill': this.props.isAnswer? 'white' : `${(this.props.primary_color)}` }}>
